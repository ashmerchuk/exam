<?php

declare(strict_types=1);

namespace DoctrineMigrations;

use Doctrine\DBAL\Schema\Schema;
use Doctrine\Migrations\AbstractMigration;

/**
 * Auto-generated Migration: Please modify to your needs!
 */
final class Version20230430212351 extends AbstractMigration
{
    public function getDescription(): string
    {
        return '';
    }

    public function up(Schema $schema): void
    {
        // this up() migrations is auto-generated, please modify it to your needs
//        $this->addSql('DROP SEQUENCE allskills_id_seq CASCADE');
//        $this->addSql('DROP SEQUENCE skills_id_seq CASCADE');
//        $this->addSql('DROP TABLE skills');
        $this->addSql("CREATE TABLE IF NOT EXISTS allSkills (id SERIAL PRIMARY KEY, nameOfTodo VARCHAR(255) NOT NULL)");

    }

    public function down(Schema $schema): void
    {
        // this down() migrations is auto-generated, please modify it to your needs
//        $this->addSql('CREATE SCHEMA public');
//        $this->addSql('CREATE SEQUENCE allskills_id_seq INCREMENT BY 1 MINVALUE 1 START 1');
//        $this->addSql('CREATE SEQUENCE skills_id_seq INCREMENT BY 1 MINVALUE 1 START 1');
        $this->addSql('CREATE TABLE skills (id SERIAL NOT NULL, name VARCHAR(255) NOT NULL, PRIMARY KEY(id))');
    }
}
